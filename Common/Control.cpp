#include "Control.h"


Control* Control::focused = NULL;

Control::~Control() {
	for each (Control* control in controls)
		if (control)
			delete control;
}

void Control::draw(Graphics& g,bool doubleborder) {
	this->drawIt(g,this, doubleborder);
	for each (Control* control in controls)
		control->draw(g,doubleborder);
}

void Control::drawIt(Graphics& g, Control* control, bool doubleborder) {
	Color oldBack = g.getBackground();
	Color oldFore = g.getForeground();
	g.setBackground(control->getBackground());
	g.setForeground(control->getForeground());
	string tmp = " ";
	tmp.resize(control->getWidth());
	for (short i = 0; i < control->getHeight(); i++)
		g.write(control->getLeft(), control->getTop() + i, tmp);
	if (control->hasBorder()) control->drawBorder(g,control, doubleborder);
	control->drawInside(g,doubleborder);
	g.setBackground(oldBack);
	g.setForeground(oldFore);
}

void Control::drawBorder(Graphics& g, Control* control, bool doubleborder){
	g.write(control->getLeft() - 1, control->getTop() - 1, "\xDA"); // Left top
	g.write(control->getLeft() + control->getWidth(), control->getTop() - 1, "\xBF"); // Right Top
	g.write(control->getLeft() - 1, control->getTop()+control->getHeight(), "\xC0"); // Left Bottom
	g.write(control->getLeft() + control->getWidth(), control->getTop()+control->getHeight(), "\xD9"); // right bottom

	for (short i = 0; i < control->getHeight(); i++) { //Vertical
		g.write(control->getLeft() - 1, control->getTop() + i, "\xB3");
		g.write(control->getLeft() + control->getWidth(), control->getTop() + i, "\xB3");
	}
	for (short i = 0; i < width; i++){ // Horizontal
		g.write(control->getLeft() + i, control->getTop() - 1, "\xC4");
		g.write(control->getLeft() + i, control->getTop() + control->getHeight(), "\xC4");
	}

	if (this->isDoubleBorder() == true) {
		int range = 1;
		g.write(control->getLeft() - 0.5 - range, control->getTop() - 0.5 - range, "\xDA"); // Left top
		g.write(control->getLeft() + control->getWidth() + range, control->getTop() - 0.5 - range, "\xBF"); // Right Top
		g.write(control->getLeft() - 0.5 - range, control->getTop() + control->getHeight() + range , "\xC0"); // Left Bottom
		g.write(control->getLeft() + control->getWidth() + range, control->getTop() + control->getHeight() + range, "\xD9"); // right bottom

		for (short i = 0; i < control->getHeight() + 2*range; i++) { //Vertical
			g.write(control->getLeft() - 1 - range, control->getTop() - range + i, "\xB3");
			g.write(control->getLeft() + control->getWidth() + range, control->getTop() - 0.5 + i, "\xB3");
		}
		for (short i = 0; i <= width + range; i++) { // Horizontal
			g.write(control->getLeft() - range + i, control->getTop() - 1 - range, "\xC4");
			g.write(control->getLeft() + i - range, control->getTop() + control->getHeight() + range, "\xC4");
		}
		g.moveTo(control->getLeft(), control->getTop());// reset cursor position
	}
}

void Control::getAllControls(vector<Control*>* controlsCopy) {
	for (size_t i = 0; i < controls.size(); i++)
		(*controlsCopy).push_back(controls[i]);
};

void Control::add(Control* newControl){
	if (newControl) {
		newControl->setParentDimensions(this->getLeft(),this->getTop());
		this->controls.push_back(newControl);
	}
}

void Control::setColor(Color background, Color foreground){
	this->background = background;
	this->foreground = foreground;
}

void Control::setConnectedControl(Control *c)
{
	connected_control = c;
}
bool Control::isInside(int x, int y,Control* control){
	x -= control->getLeft();
	y -= control->getTop(); 
	return x >= 0 && y >= 0 && x < control->getWidth() && y < control->getHeight();
}

void Control::mousePressed(int x, int y, bool isLeft) {
	if (isLeft)
		for each (Control* control in controls)
			if (control->isClick && isInside(x, y,control))
				return control->mousePressed(x, y,isLeft);
}

void Control::setFocus(int x, int y) {
	for each (Control* control in controls)
		if (control->canGetFocus() && isInside(x, y, control)) 
			setFocus(*control);
}

